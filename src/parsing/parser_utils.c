...
void check_token_syntax(t_token *token_head)
{
    ...
    if (token_head->t_type == 1)
        free_errmsg_exit(token_head, "minishell: syntax error near unexpected token `|'", 2);
    while (token_head && token_head->next)
    {
        if (token_head->t_type >= 2)
        {
            if (token_head->next->t_type >= 1)
                free_errmsg_exit(token_head, "minishell: syntax error near unexpected redirections tokens", 2);
        }
        if (token_head->t_type == 1)
        {
            if (token_head->next->t_type >= 1)
        ...
        if (token_head->t_type == 0 && token_head->quote_type == 0)
        {
            if (ft_strchr((const char *)token_head->str, ';') || ft_strchr((const char *)token_head->str, '\''))
                free_errmsg_exit(token_head, "minishell: syntax error: special characters", 2);
        }
        token_head = token_head->next;
    }
    if (token_head->t_type == 1)
        free_errmsg_exit(token_head, "minishell: syntax error not word after the token `|'", 2);
}
